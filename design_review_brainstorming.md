...
- [recommended] Several classes have grown into "workflow hubs", especially `Application`, `ConsoleUI`, `ConfiguratoreController`, `PropostaController`, `PropostaService`, `StateTransitionService`, and `BatchImportService`.
...
- [recommended] `PropostaController` and `ConfiguratoreController` contain presentation formatting and navigation logic. Controllers are allowed to coordinate flow, but formatting labels and list construction can move to view/presenter methods.
...
| `presentation/controller/AuthController.java` - `AuthController` | Login and registration UI flow. | Duplicates credential validation already in service; loops are UI-heavy. | SRP acceptable; DRY mild issue. | [recommended] Service exposes validation result/policy so controller only prompts and displays. | Controller; Strategy for credential policy optional. | recommended |
| `presentation/controller/ConfiguratoreController.java` - `ConfiguratoreController` | Main configurator menu and catalog/proposal/batch routing. | Very large; handles menu, catalog CRUD, proposal routing, withdrawal selection, initial setup; nested adapter interface. | SRP/High Cohesion weak; Controller role too broad. | [recommended] Split into `CatalogoController`, `CategoriaController`, `CampiController`, and session menu coordinator. | Command for menu actions optional. | recommended |
| `presentation/controller/ConfiguratoreController.java` - `GestioneCampi` nested interface | Local abstraction to reuse field-management menu logic. | Useful but hidden inside an already large controller; mixes UI operation adapter with catalog operations. | GRASP Polymorphism good locally; SRP pressure from host class. | [optional] Keep if controller is not split; otherwise move to package-private strategy/helper. | Strategy/Command-like adapter. | optional |
| `presentation/controller/PropostaController.java` - `PropostaController` | Proposal creation, correction, publication, bacheca/archive display flow. | Handles workflow, list labels, error-field mapping, publication UI, archive navigation. | SRP and High Cohesion weak; still a reasonable extracted controller. | [recommended] Extract proposal creation form flow and archive presenter/listing helper. | Controller; Presenter; Command optional. | recommended |
| `presentation/controller/FruitoreController.java` - `FruitoreController` | Fruitore bacheca, subscription, unsubscription, notification routing. | Formats proposal lists and calculates free seats in controller. | SRP mostly okay; some presentation logic in controller. | [optional] Move list rendering to view/presenter; keep use-case orchestration here. | Presenter optional. | optional |
| `presentation/controller/SpazioPersonaleController.java` - `SpazioPersonaleController` | Notification list/delete flow. | Formats dates directly; controller decides display layout. | SRP mild issue; MVC view formatting leak. | [optional] Move notification rendering to view; use shared date formatter. | Presenter optional. | optional |
| `presentation/controller/BatchImportController.java` - `BatchImportController` | Batch import UI flow and result display. | Formats result and errors directly; otherwise small and cohesive. | SRP acceptable; MVC formatting leak mild. | [optional] Add `stampaImportResult` to view or presenter if reports grow. | Presenter optional. | optional |
