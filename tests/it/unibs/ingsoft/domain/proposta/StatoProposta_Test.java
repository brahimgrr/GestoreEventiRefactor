...
/*
Secondo me non ha senso testare un enum. test inutile
 */
class StatoProposta_Test {
    @Test
    void valuesEValueOf_restituisconoEnumAttesi() {
        assertAll(
                () -> assertArrayEquals(new StatoProposta[]{
                        StatoProposta.BOZZA,
                ...
                }, StatoProposta.values()),
                () -> assertEquals(StatoProposta.APERTA, StatoProposta.valueOf("APERTA"))
        );
    }

    @Test
    void canTransitionTo_conDestinazioneNull_restituisceFalsePerOgniStato() {
        for (StatoProposta stato : StatoProposta.values()) {
            assertFalse(stato.canTransitionTo(null));
        }
    }
